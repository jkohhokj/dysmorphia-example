...
void load_firmware(void){
    ...
    while (1){
        ...
        for (int i = 0; i < frame_length; ++i){
            data[data_index] = (uint8_t)uart_read(UART1, BLOCKING, &read);
            data_index += 1;
        }
        // GCM decrypt
        uint8_t AAD[16] = {0x40,0x49,0x42,0x41,0x41,0x44,0x40,0x46,0x4c,0x43,0x4c,0x46,0x4a,0x47,0x47,0x46};
        int aad_len = 16;
        uint8_t auth_tag[16] = {0x40,0x49,0x42,0x41,0x41,0x44,0x40,0x46,0x4c,0x43,0x4c,0x46,0x4a,0x47,0x47,0x46};
        int res = gcm_decrypt_and_verify(AES_KEY, IV, data[data_index-256], 256, aad, add_len, auth_tag);
        ++frameNum;
        uart_write_hex_bytes(UART0,res);
